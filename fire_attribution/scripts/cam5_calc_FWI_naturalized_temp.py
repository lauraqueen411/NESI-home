...
def main():
    ...
    for scen in ['All']:#,'Nat']:
        ...
        for num in ens_list:
            ...
            if os.path.exists("/nesi/nobackup/niwa00015/queenle/CAM5/daily/naturalized/FWI/naturalized_temp_FWI_"+scen+"_run"+num_string+".nc"):
                continue
...
def calc_and_save(scen,num_string):
    ds = xr.open_dataset('/nesi/project/niwa00015/queenle/data/fire/CAM5-1-1/daily/inputs/FWI_inputs_'+scen+'_run'+num_string+'_1959-2018.nc')
    ds = ds.rename({'lat':'latitude','lon':'longitude'})
    if scen == 'All':
        nat_ta = xr.open_dataarray('/nesi/nobackup/niwa00015/queenle/CAM5/daily/naturalized/tasmax_All_run'+num_string+'_1959-2018.nc')
        nat_ta = nat_ta.rename({'lat':'latitude','lon':'longitude'})

        ds = ds.drop(['tasmax'])#,'rh'])
        ds = ds.assign(tasmax=nat_ta)#,rh=nat_rh)

    fwi_arr = rasterFWI(ds)

    ds["FWI"]=(['time', 'lev','latitude', 'longitude'],  fwi_arr)
    FWI_da = ds.drop(['pr','tasmax','rh','wind'])
    FWI_da.to_netcdf("/nesi/nobackup/niwa00015/queenle/CAM5/daily/naturalized/FWI/naturalized_temp_FWI_"+scen+"_run"+num_string+".nc")

def rasterFWI(ds):
    
    lat = np.array([[lat]*len(ds.longitude.values) for lat in ds.latitude.values])

    mask = np.isnan(ds.isel(time=0).tasmax.values)
...
def flatten_inputs(data_input): 
    ...
    """
    T = np.ravel(data_input.tasmax.values)
    RH = np.ravel(data_input.rh.values)
    RH = np.where(RH>100, 100, RH)
    U = np.ravel(data_input.wind.values)
    PR = np.ravel(data_input.pr.values)
    return(T, RH, U, PR)
