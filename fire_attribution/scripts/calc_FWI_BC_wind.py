...
def calc_and_save(scen,num_string):
    ...
    fwi_arr = rasterFWI(ds,BC_wind_da)

    ds["FWI"]=(['time', 'lev','latitude', 'longitude'],  fwi_arr)
    FWI_da = ds.drop(['precipi','ta','rha','wind'])
    FWI_da.to_netcdf("/nesi/project/niwa00015/queenle/data/fire/d4pdf/FWI/daily/BC_wind_FWI/BC_wind_FWI_"+scen+"_m"+num_string+".nc")

def rasterFWI(ds,BC_wind_da):
    ...
    for day in ds.time:
        ...
        day_wind_da = BC_wind_da.sel(time=day)
        month = int(day.dt.month)

        (T, RH, U, PR) = flatten_inputs(day_ds, day_wind_da)
