...
def calc_and_save(scen,num_string):
    ds = xr.open_dataset("/nesi/project/niwa02986/queenle/data/d4pdf/WNAmerica/FWI/daily/inputs/FWI_inputs_"+scen+"_m"+num_string+"_1951-2021.nc")
    ds = ds.drop('wind')
    ds = ds.rename({'lat':'latitude','lon':'longitude'})

    BC_wind_da = xr.open_dataarray('/nesi/project/niwa02986/queenle/data/d4pdf/WNAmerica/FWI/daily/inputs/BC_wind/BC_WIND_'+scen+'_m'+num_string+'_1951-2021.nc').drop(['latitude','longitude'])
    BC_wind_da = BC_wind_da.rename({'lat':'latitude','lon':'longitude'})
    ds = ds.assign(wind=BC_wind_da)

    if scen == 'HPB':
        nat_ta = xr.open_dataarray('/nesi/nobackup/niwa00015/queenle/d4PDF/daily/naturalized/ta_HPB_m'+num_string+'_1951-2021.nc')
        #nat_rha = xr.open_dataarray('/nesi/nobackup/niwa00015/queenle/d4PDF/daily/naturalized/rha_HPB_m'+num_string+'_1951-2021.nc')

        ds = ds.drop(['ta'])#,'rha'])
        ds = ds.assign(ta=nat_ta)#,rha=nat_rha)
    
    fwi_arr = rasterFWI(ds)

    ds["FWI"]=(['time', 'lev','latitude', 'longitude'],  fwi_arr)
    FWI_da = ds.drop(['precipi','ta','rha','wind'])
    FWI_da.to_netcdf("/nesi/nobackup/niwa00015/queenle/d4PDF/daily/naturalized/FWI/naturalized_temp_FWI_"+scen+"_m"+num_string+".nc")
...
def flatten_inputs(ds): 
    ...
    """
    T = np.ravel(ds.ta.values)
    RH = np.ravel(ds.rha.values)
    RH = np.where(RH>100, 100, RH)
    U = np.ravel(ds.wind.values)
    PR = np.ravel(ds.precipi.values)
    return(T, RH, U, PR)
